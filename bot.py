...
async def menu_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "cmd_download":
        ...
        await query.message.reply_text(
            ...
            "You can also use custom code: <code>/short url mycode</code>\n\n"
            "📊 Check clicks anytime: <code>/stats mycode</code>",
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_menu()
        )
...
async def short_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        msg = (
            ...
            f"Short: {short_link}\n\n"
            f"📊 Clicks are actually tracked in the database.\n"
            f"Check anytime with: <code>/stats {code}</code>"
        )
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args:
        await update.message.reply_text(
            ...
            "Usage:\n"
            "/stats abc123\n\n"
            "Shows how many times people clicked the short link.\n"
            "Tracking happens live on the server when someone visits /s/xxx",
            parse_mode=ParseMode.HTML,
        )
        return
        ...
        if long_url:
            try:
                ...
                await safe_reply(
                    ...
                    f"📊 Clicks tracked. Check with <code>/stats {code}</code>"
                )
            except ValueError as ve:
