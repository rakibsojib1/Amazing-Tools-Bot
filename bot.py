...
async def menu_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "cmd_download":
        ...
        await query.message.reply_text(
            ...
            "• Send any photo → new buttons will appear for:\n"
            "  Compress, WebP, Resize, PDF, Filters\n\n"
            "Commands:\n"
            "/compress • /webp • /resize • /pdf\n\n"
            "Or use the beautiful Mini App for quality selection & presets.",
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_menu()
        )
...
async def _handle_image_command(update: Update, context: ContextTypes.DEFAULT_TYPE, op: str):
    ...
    if op == "compress" and context.args:
        try:
            ...
            params["quality"] = 80
    elif op == "resize" and len(context.args) >= 2:
        try:
...
async def auto_detect(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if msg.photo:
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [
                InlineKeyboardButton("📐 Resize 1280px", callback_data="img:resize:1280"),
        ...
        await update.message.reply_text(
            "📸 <b>Photo received!</b>\nChoose what to do (or use Mini App for more options):",
