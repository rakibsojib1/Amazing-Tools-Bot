...
async def images_to_pdf(photos: list, margin_mode: str = "normal", layout: str = "portrait") -> bytes:
    """Convert list of photos to a clean multi-page PDF.
    margin_mode: 'none' (full bleed - images fill entire pages back-to-back, nothing else visible),
                 'small', or 'normal' (with margins, centered).
    layout: 'portrait' (default) or 'landscape'.
    """
    if not photos:
    ...
    # A4-ish page size in pixels (approx 150 DPI)
    if str(layout).lower().startswith(("l", "land")):
        PAGE_W, PAGE_H = 1754, 1240  # landscape
    else:
        PAGE_W, PAGE_H = 1240, 1754  # portrait

    pages = []
    for img in images:
        if margin_mode == "none":
            # Full bleed cover: crop image to page aspect ratio then resize exactly to fill page
            page_ratio = PAGE_W / float(PAGE_H)
            img_ratio = img.width / float(img.height) if img.height else 1.0
            if img_ratio > page_ratio:
                # crop horizontal sides
                new_w = int(img.height * page_ratio)
                left = (img.width - new_w) // 2
                img_c = img.crop((left, 0, left + new_w, img.height))
            else:
                # crop top/bottom
                new_h = int(img.width / page_ratio)
                top = (img.height - new_h) // 2
                img_c = img.crop((0, top, img.width, top + new_h))
            page_img = img_c.resize((PAGE_W, PAGE_H), Image.LANCZOS)
            pages.append(page_img)
        else:
            margin = 30 if margin_mode == "small" else 60
            avail_w = PAGE_W - 2 * margin
            avail_h = PAGE_H - 2 * margin
            if avail_w < 10 or avail_h < 10:
                page_img = img.resize((PAGE_W, PAGE_H), Image.LANCZOS)
            else:
                ...
                x = (PAGE_W - new_w) // 2
                y = (PAGE_H - new_h) // 2
                page_img.paste(resized, (x, y))
            pages.append(page_img)

    buf = io.BytesIO()
    if pages:
        title = "Amazing Tools - PDF (No Margin, Full Bleed)" if margin_mode == "none" else "Amazing Tools - PDF"
        pages[0].save(
            buf,
            "PDF",
            resolution=150.0,
...
async def pdf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not photos:
        await update.message.reply_text(
            "📄 <b>PDF from Images</b>\n\n"
            "Send 1+ photos, reply with /pdf or use after send.\n"
            "Or send photo → tap 'PDF Options →' for margin & layout choices (No Margin = full page images back to back).",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    try:
        ...
        pdf_bytes = await images_to_pdf(photos, margin_mode="none", layout="portrait")
        await context.bot.send_document(
            chat_id=update.effective_chat.id,
            document=pdf_bytes,
            filename="amazing_tools_images.pdf",
            caption="📄 PDF (No Margin - Full Bleed) • Amazing Tools",
        )
        await status.delete()
...
async def image_tools_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if op == "resize" and len(data) > 2 and data[2] == "custom":
    ...
    # PDF options menu (select margin + layout)
    if op == "pdf" and len(data) == 2:
        # Initial "PDF Options →" clicked → show choices
        pdf_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("🖼️ No Margin (Full Bleed)", callback_data="img:pdf:none:portrait"),
            ],
            [
                InlineKeyboardButton("📐 Small Margin", callback_data="img:pdf:small:portrait"),
                InlineKeyboardButton("📄 Normal Margin", callback_data="img:pdf:normal:portrait"),
            ],
            [
                InlineKeyboardButton("↔️ Landscape No Margin", callback_data="img:pdf:none:landscape"),
            ],
        ])
        await query.message.edit_text(
            "📄 <b>PDF Options — Margin & Page Layout</b>\n\n"
            "No Margin = images one after another, fill the full page completely (no white space, no other content).\n\n"
            "Choose style:",
    ...
    try:
        ...
        if op == "pdf":
            # Detailed callback e.g. img:pdf:none:portrait
            margin_mode = "normal"
            layout = "portrait"
            if len(data) > 2:
                margin_mode = data[2]
            if len(data) > 3:
                layout = data[3]
            pdf_bytes = await images_to_pdf([photo], margin_mode=margin_mode, layout=layout)
            caption = "📄 PDF (No Margin Full Bleed)" if margin_mode == "none" else "📄 Your images as PDF • Amazing Tools"
            await context.bot.send_document(
                chat_id=update.effective_chat.id,
                document=pdf_bytes,
    ...
    if msg.photo:
        ...
        if pending_tool:
            ...
            try:
                ...
                if tool == "pdf":
                    pdf_bytes = await images_to_pdf([msg.photo[-1]], margin_mode="none", layout="portrait")
                    await context.bot.send_document(
                        chat_id=update.effective_chat.id,
                        document=pdf_bytes,
                        filename="amazing_tools.pdf",
                        caption="📄 PDF (No Margin Full Bleed) from Mini App",
                    )
                else:
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [
                ...
                InlineKeyboardButton("📄 PDF Options →", callback_data="img:pdf"),
            ],
            [
...
class MiniAppRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Proxy download to avoid 403 from external hosts (Varnish cache / hotlink protections)
        if parsed.path == '/proxy-download':
            target = qs.get('url', [''])[0]
            ref = qs.get('ref', [''])[0] or ''
            if target:
                try:
                    # Build realistic headers. Use source page as referer when available (very effective vs 403)
                    ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36'
                    headers = {
                        'User-Agent': ua,
                        'Accept': 'video/webm,video/ogg,video/*;q=0.9,application/ogg;q=0.7,audio/*;q=0.6,*/*;q=0.5',
                        'Accept-Language': 'en-US,en;q=0.9',
                        'Referer': ref if ref.startswith('http') else 'https://www.youtube.com/',
                    }
                    ...
                    with urllib.request.urlopen(req, timeout=90) as resp:
                        ...
                        # Try to get a decent filename from headers or default
                        cdisp = resp.headers.get('Content-Disposition', '')
                        fname = 'video.mp4'
                        if 'filename=' in cdisp:
                            try:
                                fname = cdisp.split('filename=')[1].strip("'\"")
                            except:
                                pass
                    ...
                    # Send clean error instead of leaking host pages
                    self.send_response(502)
                    self.send_header('Content-Type', 'application/json')
                    self.send_header('Access-Control-Allow-Origin', '*')
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": f"proxy_fetch_failed", "detail": str(e)[:120]}).encode('utf-8'))
                    return
