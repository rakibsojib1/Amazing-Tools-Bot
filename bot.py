...
async def perform_download(
    ...
    if not filepath:
        await status_msg.edit_text(
            ...
            "Try another link (YouTube, TikTok, Instagram Reels usually work great)."
        )
        return
    ...
    await status_msg.edit_text(f"📤 Uploading to you... ({size_mb:.1f} MB)")
    ...
    try:
        ...
        logger.error(f"Send error ({size_mb:.1f}MB): {exc}")
        await status_msg.edit_text(
            f"✅ Downloaded ({size_mb:.1f} MB) but couldn't send to you.\n"
            "File might be too large for Telegram right now."
        )
    finally:
...
def run_miniapp_server(host: str = "0.0.0.0", port: int = 8080) -> None:
    ...
    # Use ThreadingHTTPServer for better handling
